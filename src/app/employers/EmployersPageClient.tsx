...
export function EmployersPageClient({ employers }: EmployersPageClientProps) {
  ...
  const filteredEmployers = useMemo(() => {
    ...
    if (magnetOnly) result = result.filter(e => e.magnet_status);
    if (unionOnly) result = result.filter(e => e.union);
    if (newGradOnly) result = result.filter(e => e.new_grad_friendly);
            ...
            <CardContent className="p-6">
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div>
                  ...
                  <div className="space-y-3 mt-2">
                    <div className="flex items-center space-x-2">
                      <Checkbox id="magnet" checked={magnetOnly} onCheckedChange={(checked) => setMagnetOnly(checked as boolean)} />
                      <label htmlFor="magnet" className="text-sm cursor-pointer">Magnet Designated</label>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Checkbox id="union" checked={unionOnly} onCheckedChange={(checked) => setUnionOnly(checked as boolean)} />
                      <label htmlFor="union" className="text-sm cursor-pointer flex items-center gap-1">
                        <Users className="w-3 h-3" /> Union Hospital
                      </label>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Checkbox id="newgrad" checked={newGradOnly} onCheckedChange={(checked) => setNewGradOnly(checked as boolean)} />
                ...
                <div>
                  ...
                  <div className="space-y-3 mt-2">
                    <div className="flex items-center space-x-2">
                      <Checkbox id="day" checked={selectedShifts.includes('day')} onCheckedChange={() => toggleShift('day')} />
                      <label htmlFor="day" className="text-sm cursor-pointer flex items-center gap-1">
                        <Sun className="w-3 h-3" /> Day Shift
                      </label>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Checkbox id="night" checked={selectedShifts.includes('night')} onCheckedChange={() => toggleShift('night')} />
                      <label htmlFor="night" className="text-sm cursor-pointer flex items-center gap-1">
                        <Moon className="w-3 h-3" /> Night Shift
                      </label>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Checkbox id="rotating" checked={selectedShifts.includes('rotating')} onCheckedChange={() => toggleShift('rotating')} />
