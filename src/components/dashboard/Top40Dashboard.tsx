...
export function Top40Dashboard() {
    const [employers, setEmployers] = useState<Top40Employer[]>([]);
    const [filters, setFilters] = useState<Top40Filters>({ states: [], departments: [], positions: [] });
    ...
    }, [state, department, position, sortBy]);

    useEffect(() => {
        fetchData();
    }, [fetchData]);

    // For cattiness, lower is better so reverse the sort
    const displayEmployers = sortBy === 'avg_cattiness'
        ? [...employers].sort((a, b) => {
            if (a.avg_cattiness === null) return 1;
            if (b.avg_cattiness === null) return -1;
            return a.avg_cattiness - b.avg_cattiness; // ascending for cattiness
        })
        : employers;
                ...
                {loading ? (
                    ...
                    <div className="space-y-3">
                        {displayEmployers.map((emp, idx) => (
                            <Link
                                key={emp.employer_id}
                                href={`/employers/${emp.employer_id}`}
                                className="block"
                            >
                                        ...
                                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-1.5 text-xs">
                                            <div className="flex items-center justify-between gap-2">
                                                <span className="text-muted-foreground flex items-center gap-1">
                                                    <Star className="w-3 h-3 text-yellow-500" /> Overall
                                                </span>
                                                <RatingBar value={emp.avg_overall} />
                                            </div>
                                            <div className="flex items-center justify-between gap-2">
                                                <span className="text-muted-foreground flex items-center gap-1">
                                                    <Users className="w-3 h-3 text-blue-500" /> Staffing
                                                </span>
                                                <RatingBar value={emp.avg_staffing} color="bg-blue-500" />
                                            </div>
                                            <div className="flex items-center justify-between gap-2">
                                                <span className="text-muted-foreground flex items-center gap-1">
                                                    <Shield className="w-3 h-3 text-green-500" /> Safety
                                                </span>
                                                <RatingBar value={emp.avg_safety} color="bg-green-500" />
                                            </div>
                                            <div className="flex items-center justify-between gap-2">
                                                <span className="text-muted-foreground flex items-center gap-1">
                                                    <Heart className="w-3 h-3 text-pink-500" /> Culture
                                                </span>
                                                <RatingBar value={emp.avg_culture} color="bg-pink-500" />
                                            </div>
                                            <div className="flex items-center justify-between gap-2">
                                                <span className="text-muted-foreground flex items-center gap-1">
                                                    <Briefcase className="w-3 h-3 text-purple-500" /> Mgmt
                                                </span>
                                                <RatingBar value={emp.avg_management} color="bg-purple-500" />
                                            </div>
                                            <div className="flex items-center justify-between gap-2">
                                                <span className="text-muted-foreground flex items-center gap-1">
                                                    <DollarSign className="w-3 h-3 text-emerald-500" /> Pay
                                                </span>
                                            ...
                                            {emp.avg_cattiness !== null && (
                                                <div className="flex items-center justify-between gap-2">
                                                    <span className="text-muted-foreground flex items-center gap-1">
                                                        <Cat className="w-3 h-3 text-orange-500" /> Cattiness
                                                    </span>
